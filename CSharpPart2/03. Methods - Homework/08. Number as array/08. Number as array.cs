...
class NumberAsArray
{
    static void Main()
    {
        Console.Write("Enter the first number [1..10 000]: ");
        string first = Console.ReadLine();

        Console.Write("Enter the second number [1..10 000]: ");
        string second = Console.ReadLine();

        if (IsCorrectNumber(first) && IsCorrectNumber(second))
        {
            List<int> result = NumbersAsArrays(first, second);
            Console.Write("\nResult: ");
            PrintResult(result);
        }
        else
        {
            Console.WriteLine(" You have entered invalid number(s)");
        }
    }
    ...
    static List<int> NumbersAsArrays(string first, string second)
    {
